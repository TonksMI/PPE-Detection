"""
PPE Detection — Traditional ML Models (SVM, Random Forest, GBM)
Fast version: runs in under 5 minutes
"""
...
print("="*60)
print("PPE DETECTION — ML PIPELINE")
print("="*60)
        ...
        for obj in root.findall("object"):
            ...
            if x2>x1 and y2>y1:
                records.append({"img_path":img_path,"class":cls,"binary":BINARY_MAP[cls],
                                 "x1":x1,"y1":y1,"x2":x2,"y2":y2,"iw":iw,"ih":ih,
                                 "coverage":(x2-x1)*(y2-y1)/(iw*ih+1e-6)})
    except: pass

df = pd.DataFrame(records)
print(f"Parsed {len(df)} boxes from {df['img_path'].nunique()} images")
...
features, y_multi, y_binary, meta = [], [], [], []
...
for _, row in df_shuf.iterrows():
    ...
    try:
        ...
        class_counts[cls] += 1; meta.append(row)
    except: pass

X = np.array(features)
y_m = np.array(y_multi); y_b = np.array(y_binary)
print(f"Feature matrix: {X.shape}")
print(f"Class balance: {dict(zip(*np.unique(y_m, return_counts=True)))}")

# Save metadata
pd.DataFrame(meta).assign(label_multi=y_multi, label_binary=y_binary).to_csv(
    os.path.join(OUT_DIR,"crop_metadata.csv"), index=False)
...
print(f"\nTrain={len(Xtr)} | Test={len(Xte)} | Classes={list(le_m.classes_)}")
...
def run(name, pipe, Xtr, Xte, ytr, yte, le, task):
    ...
    acc=rpt['accuracy']
    print(f"  [{name:25s}] acc={acc:.4f}  train={tt:.1f}s")
...
# SVM
results['svm_multi']  = run("SVM (multi)",
    Pipeline([('sc',StandardScaler()),('pca',PCA(100,random_state=42)),
              ('svm',SVC(kernel='rbf',C=10,gamma='scale',probability=True,random_state=42))]),
    Xtr,Xte,yt,yv,le_m,"multi")
results['svm_binary'] = run("SVM (binary)",
    Pipeline([('sc',StandardScaler()),('pca',PCA(80,random_state=42)),
              ('svm',SVC(kernel='rbf',C=5,gamma='scale',probability=True,random_state=42))]),
    Xtr_b,Xte_b,yt_b,yv_b,le_b,"binary")

# Random Forest
results['rf_multi']   = run("Random Forest (multi)",
    Pipeline([('sc',StandardScaler()),
              ('rf',RandomForestClassifier(200,max_depth=15,min_samples_split=4,
                                           n_jobs=-1,random_state=42))]),
    Xtr,Xte,yt,yv,le_m,"multi")
results['rf_binary']  = run("Random Forest (binary)",
    Pipeline([('sc',StandardScaler()),
              ('rf',RandomForestClassifier(200,max_depth=12,n_jobs=-1,random_state=42))]),
    Xtr_b,Xte_b,yt_b,yv_b,le_b,"binary")

# Gradient Boosting
results['gbm_multi']  = run("GradBoost (multi)",
    Pipeline([('sc',StandardScaler()),('pca',PCA(80,random_state=42)),
              ('gbm',HistGradientBoostingClassifier(max_iter=100,max_depth=6,
                                                    learning_rate=0.1,random_state=42))]),
    Xtr,Xte,yt,yv,le_m,"multi")
results['gbm_binary'] = run("GradBoost (binary)",
    Pipeline([('sc',StandardScaler()),('pca',PCA(60,random_state=42)),
...
for ax, task_key, title in [
    (axes[0],"multi","Multi-class Accuracy"),
    (axes[1],"binary","Binary Accuracy"),
]:
    res_t = {k:v for k,v in results.items() if v['task']==task_key}
    names=[v['name'] for v in res_t.values()]
    accs=[v['accuracy'] for v in res_t.values()]
    cols=['#e74c3c','#3498db','#2ecc71'][:len(names)]
...
plt.suptitle("PPE Detection — Model Comparison",fontsize=15,fontweight='bold')
plt.tight_layout()
...
plt.suptitle("Confusion Matrices — Multi-class PPE Detection",fontsize=14,fontweight='bold')
plt.tight_layout()
...
for (k,res),col in zip(binary_res.items(),['#e74c3c','#3498db','#2ecc71']):
    if hasattr(res['model'],'predict_proba'):
        ...
        fpr,tpr,_=roc_curve(yv_b,ys)
        ax.plot(fpr,tpr,color=col,lw=2,label=f"{res['name']} (AUC={auc(fpr,tpr):.3f})")
...
ax.set_title("RF Feature Importances (Top 30)",fontsize=12,fontweight='bold')
plt.tight_layout()
